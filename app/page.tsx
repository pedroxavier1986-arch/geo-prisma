...
export default function Home() {
 ...
 return <div className="app-shell">
  ...
  <main>
   ...
   <section id="workspace" role="tabpanel" aria-labelledby={tab === 'new' ? 'new-tab' : 'history-tab'} className="workspace">
    {tab === 'new' ? <>
     ...
     <div className="workspace-body"><div className="upload-panel">
      ...
      {inspection && inspection.invalid > 0 && !running && <p className="warning"><AlertCircle size={16}/>{number(inspection.invalid)} registros com endereço, CEP ou cidade inválidos serão listados no relatório.</p>}
   ...
   {inspection && tab === 'new' && <section className="preview-section"><div className="preview-heading"><h3>Uma prévia da sua base</h3><span>Primeiros {Math.min(5, inspection.rows.length)} de {number(inspection.total)} registros</span></div><div className="table-scroll"><table><thead><tr><th>Estabelecimento</th><th>Endereço</th><th>Número</th><th>CEP</th><th>Cidade</th><th>Latitude</th><th>Longitude</th><th>Status</th></tr></thead><tbody>{inspection.rows.slice(0, 5).map(row => { const result = results.find(r => r.key === row.key); const existing = row.latitude !== null && row.longitude !== null; return <tr key={row.row}><td>{row.name}</td><td>{row.address || '—'}</td><td>{row.number || '—'}</td><td>{row.cep}</td><td>{row.city}</td><td>{(existing ? row.latitude : result?.latitude)?.toFixed(6) || '—'}</td><td>{(existing ? row.longitude : result?.longitude)?.toFixed(6) || '—'}</td><td><span className={`row-status ${existing || result?.status === 'success' ? 'ok' : ''}`}>{existing ? 'Preservado' : row.issue ? 'Revisar' : result?.status === 'success' ? 'Preenchido' : finished ? 'Não encontrado' : 'Pendente'}</span></td></tr>; })}</tbody></table></div></section>}
  ...
  <dialog ref={dialog} onCancel={() => setHelp(false)} onClick={e => { if (e.target === dialog.current) setHelp(false); }} aria-labelledby="help-title"><div className="dialog-heading"><h2 id="help-title">Do Excel ao mapa.</h2><button className="icon-button" onClick={() => setHelp(false)} aria-label="Fechar ajuda"><X size={21}/></button></div><ol className="help-list"><li><strong>Importe sua base</strong><p>Use um .xlsx de até 5 MB, com a aba Base e as colunas address, number, cep, city, latitude e longitude. Também aceitamos endereco, endereço, street ou logradouro no lugar de address.</p></li><li><strong>Confira e geocodifique</strong><p>Consultamos cada combinação de logradouro, número, CEP e cidade. A duração depende do tamanho da base e da disponibilidade do serviço.</p></li><li><strong>Baixe o resultado</strong><p>As coordenadas completas são preservadas. Uma aba de relatório lista o que precisa de revisão. Confira a precisão antes de usar os pontos em análises.</p></li></ol><div className="help-footnote">O endereço é combinado com CEP e cidade para refinar o ponto geográfico. Geocodificação: Photon. Dados © <a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noreferrer">OpenStreetMap</a>.</div><button className="primary-button" onClick={() => setHelp(false)}>Entendi <Check size={17}/></button></dialog>
 </div>;
}

...
export default function Home() {
 ...
 return <div className="app-shell">
  ...
  <main>
   ...
   {inspection && tab === 'new' && <section className="preview-section"><div className="preview-heading"><h3>Uma prévia da sua base</h3><span>Primeiros {Math.min(5, inspection.rows.length)} de {number(inspection.total)} registros</span></div><div className="table-scroll"><table><thead><tr><th>Estabelecimento</th><th>Endereço</th><th>CEP</th><th>Cidade</th><th>Latitude</th><th>Longitude</th><th>Status</th></tr></thead><tbody>{inspection.rows.slice(0, 5).map(row => { const result = results.find(r => r.key === row.key); const existing = row.latitude !== null && row.longitude !== null; return <tr key={row.row}><td>{row.name}</td><td>{row.address || '—'}</td><td>{row.cep}</td><td>{row.city}</td><td>{(existing ? row.latitude : result?.latitude)?.toFixed(6) || '—'}</td><td>{(existing ? row.longitude : result?.longitude)?.toFixed(6) || '—'}</td><td><span className={`row-status ${existing || result?.status === 'success' ? 'ok' : ''}`}>{existing ? 'Preservado' : row.issue ? 'Revisar' : result?.status === 'success' ? 'Preenchido' : finished ? 'Não encontrado' : 'Pendente'}</span></td></tr>; })}</tbody></table></div></section>}

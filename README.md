...
- Colunas `address` (ou `endereco`, `endereço`, `street` ou `logradouro`), `number` (ou `numero`, `número` ou `house_number`), `cep` (ou `postcode`), `city`, `latitude`, `longitude`.

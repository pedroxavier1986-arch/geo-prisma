...
test('normalizes numeric CEPs and rejects malformed values', () => {
 ...
 assert.equal(placeKey('01311100','  SÃO  PAULO ','Rua A'),placeKey('01311100','São Paulo','Rua A'));
});
test('deduplicates places, repairs incomplete pairs, preserves complete data and other sheets', async () => {
 ...
 base.addRow(['name','postcode','city','address','latitude','longitude','custom']);
 ...
 assert.equal(output.getWorksheet('Base')!.getCell('E2').value,-23.56);
 assert.equal(output.getWorksheet('Base')!.getCell('F3').value,-46.65);
 assert.equal(output.getWorksheet('Base')!.getCell('E4').value,-21);
 assert.equal(output.getWorksheet('Base')!.getCell('F4').value,-42);
 ...
 assert.equal(inspection.sheet.getCell('E2').value,null);
...
test('rejects missing Base and incomplete headers', async () => {
 ...
 await assert.rejects(inspectWorkbook(await w2.xlsx.writeBuffer() as unknown as ArrayBuffer),/address, latitude, longitude/);
});

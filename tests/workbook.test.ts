...
test('normalizes numeric CEPs and rejects malformed values', () => {
 ...
 assert.equal(placeKey('01311100','  SÃO  PAULO '),placeKey('01311100','São Paulo'));
});
test('deduplicates places, repairs incomplete pairs, preserves complete data and other sheets', async () => {
 ...
 base.addRow(['name','postcode','city','latitude','longitude','custom']);
 base.addRow(['A','01311100','São Paulo',null,null,'keep']);
 base.addRow(['B',1311100,'SÃO PAULO',-20,null,'keep']);
 base.addRow(['C','01311100','São Paulo',-21,-42,'keep']);
 base.addRow(['D','broken','São Paulo',null,null,'keep']);
 ...
 assert.equal(output.getWorksheet('Base')!.getCell('D2').value,-23.56);
 assert.equal(output.getWorksheet('Base')!.getCell('E3').value,-46.65);
 assert.equal(output.getWorksheet('Base')!.getCell('D4').value,-21);
 assert.equal(output.getWorksheet('Base')!.getCell('E4').value,-42);
 assert.equal(output.getWorksheet('Base')!.getCell('F2').font.bold,true);
 ...
 assert.equal(inspection.sheet.getCell('D2').value,null);
...
test('rejects missing Base and incomplete headers', async () => {
 ...
 await assert.rejects(inspectWorkbook(await w2.xlsx.writeBuffer() as unknown as ArrayBuffer),/latitude, longitude/);
});

...
export type Place = { key: string; cep: string; city: string };
export type PreviewRow = { row: number; name: string; cep: string; city: string; latitude: number | null; longitude: number | null; key: string; issue?: string };
...
export function placeKey(cep: string, city: string): string {
 return `${cep}|${city.normalize('NFD').replace(/[\u0300-\u036f]/g, '').trim().replace(/\s+/g, ' ').toLowerCase()}`;
}
...
export async function inspectWorkbook(data: ArrayBuffer): Promise<Inspection> {
 ...
 const missing = ['cep', 'city', 'latitude', 'longitude'].filter(k => !columns[k]);
 ...
 sheet.eachRow((row, i) => {
  ...
  const key = placeKey(cep, city);
  const complete = latitude !== null && longitude !== null;
  const issue = !complete && (!cep || !city) ? (!cep ? 'CEP inválido ou ausente' : 'Cidade ausente') : undefined;
  if (complete) existing++; else { pending++; if (issue) invalid++; else places.set(key, { key, cep, city }); }
  const nameColumn = columns.name || columns.nome;
  rows.push({ row: i, name: nameColumn ? row.getCell(nameColumn).text : `Registro ${i - 1}`, cep: cep || row.getCell(columns.cep).text, city, latitude, longitude, key, issue });
 });
...
export async function templateWorkbook() {
 ...
 sheet.columns = ['name', 'cep', 'city', 'latitude', 'longitude'].map(key => ({ header: key, key, width: key === 'name' ? 30 : 20 }));
 sheet.getColumn('cep').numFmt = '@';
 sheet.addRow({ name: 'Estabelecimento de exemplo', cep: '01311100', city: 'São Paulo' });
